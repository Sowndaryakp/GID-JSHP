  ...
  <div>
    ...
    <div class="container mx-auto" ref="reportContainerTable">
      ...
      <!-- <div class="flex justify-center items-center mt-4">
        <button @click="prevPage" :disabled="currentPage === 1" class="bg-blue-500 text-white px-4 py-2 rounded mr-2">Previous</button>
        <span>Page {{ currentPage }} of {{ totalPages }}</span>
        <button @click="nextPage" :disabled="currentPage === totalPages" class="bg-blue-500 text-white px-4 py-2 rounded ml-2">Next</button>
      </div> -->
      <div class="flex justify-center items-center mt-4">
...
const validateStandardCurrent = () => {
  ...
  if (!currentPattern.test(elementCurrent.value)) {
    ...
    currentError.value = ''; // Reset the error message even if the input is correct
  }
};
...
const validateStandardVoltage = () => {
  const voltagePattern = /^\d{2}-\d{2}$|^\d{2}-\d{2}$/;

  if (!voltagePattern.test(elementVoltage.value)) {
    voltageError.value = 'You have to enter the format as 12-34 or 12-34';
  } else {
    voltageError.value = ''; // Reset the error message even if the input is correct
  }
};
...
const paginatedData = computed(() => {
  ...
  // Sort tableData.value by type alphabetically
  const sortedData = tableData.value.slice().sort((a, b) => a.type.localeCompare(b.type));
