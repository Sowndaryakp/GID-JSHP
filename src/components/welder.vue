        ...
        <table class="w-full" >
          <thead >
            <tr class="bg-blue-500 text-white">
               ...
               <th class="px-4 py-2 font-poppins">Welding Machine</th>
               <th class="px-16  py-2 font-poppins">Operator</th>
               <th class="px-14 py-2 font-poppins">Plate</th>
               <th class="px-32 py-2 font-poppins">Element</th>
               <th class="px-8 py-2 font-poppins">STANDARD</th>
               <th class="px-16 py-2 font-poppins ">ACTUAL</th>
               <th class="px-10 py-2 font-poppins">Live</th>
               <th class="px-24 py-2 font-poppins">Project</th>
             ...
             <tr v-for="row in dataRows" :key="row.id" class="border-t hover:bg-white transition-colors">
               ...
               <td class="flex flex-col items-center sm:px-4 sm:py-16 font-poppins">
                  <div class="flex flex-col items-center font-poppins pt-16">
                    ...
                    <div  v-if="!row.isEditing" type="text" class="border text-lg text-blue-800 font-bold border-gray-300 rounded-md w-26 px-2 py-2 sm:px-4 sm:py-2 mb-2 w-40 text-center">{{ row.plate }}</div>
                    ...
                    <div  v-if="!row.isEditing" type="text" class="border text-lg text-blue-800 font-bold border-gray-300 rounded-md w-26 px-2 py-2 sm:px-4 sm:py-2 mb-2 w-40 text-center">{{ row.platediscription }}</div>
               ...
               <td>
                 <div class=" flex  flex-col items-center font-poppins ">
                  <div  v-if="!row.isEditing" class="text-center"><td class="border border-gray-300  px-4 py-1 rounded-md mb-4 w-24">{{ row.element }}</td></div>
                  <div v-else>
                   <select v-model="row.element" @change="onOptionSelected($event)"
                     class="border border-gray-300  px-4 py-1 rounded-md mb-4">
                 ...
                 <div  v-if="!row.isEditing" type="text" class="border border-gray-300 rounded-md w-26 px-2 py-2 sm:px-4 sm:py-2 mb-2 w-40 text-center"><b> {{ row.element_description || 'N/A' }}</b></div>
                 ...
                 <div  v-if="!row.isEditing" type="text" class="border border-gray-300 rounded-md w-26 px-2 py-2 sm:px-4 sm:py-2 mb-2 w-40 text-center"><b> {{ row.range || 'N/A' }}</b></div>
...
const handleSubmit = async (machineId) => {
  try {
    ...
    const url = `http://172.18.100.240:6969/all_logs/${machineId}/${startDate.value}/${endDate.value}`;
...
onMounted(async () => {
  ...
  for (const row of dataRows.value) {
    try {
      ...
      const welderResponse = await fetch(`http://172.18.100.240:6969/welder/${mostRecentOperatorName}`);
...
;
      ...
      const elementResponse = await fetch(`http://172.18.100.240:6969/elements/${mostRecentElementType}`);
      const elementData = await elementResponse.json();
...
const connectToWebSocket = () => {
  const socket = new WebSocket('ws://172.18.100.240:6969/live_recent_ws');
...
async function saveEditedData(row) {
  try {
    ...
    const response = await axios.put(`http://172.18.100.240:6969/edit/${row.element}/${row.operator}?machine_id=${row.itemno}`, {
...
function showUserPopup(machineId) {
  ...
  axios.get(`http://172.18.100.240:6969/live_data/live_type/${machineId}`)
    .then(response => {
...
//   try {
//     console.log(machineId)
//     const response = await axios.put(`http://172.18.100.240:6969/edit/${machineId.element}/${machineId.operator}`);
...
const fetchDatastate = async () => {
  try {
    const response = await axios.get('http://172.18.100.240:6969/machine_color/');
