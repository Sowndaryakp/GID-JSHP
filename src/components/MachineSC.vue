...
const fetchElementTypes = async () => {
  const elementsUrl = 'http://172.18.100.240:6969/elements'; // Replace with the actual endpoint
  try {
...
const fetchI_noValues = async () => {
  const url = 'http://172.18.100.240:6969/project/';
  try {
...
const deleteData = (machineName, startTime, endTime, elementName, operatorName) => {
  ...
  const url = `http://172.18.100.240:6969/op_shift/?machine_name=${machineName}&start_time=${startTimeEpoch}&end_time=${endTimeEpoch}&element_name=${encodedElementName}&operator_name=${encodedOperatorName}`;
...
const generateProductionExcel = async () => {
  ...
  const backendURL = `http://172.18.100.240:6969/graph/get_production_end_data?machine_id=${machine_id.value}&start_date=${formattedStartDateProd}&end_date=${formattedEndDateProdProd}`;
